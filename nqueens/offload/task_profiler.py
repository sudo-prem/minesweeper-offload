    ...
    def get_ic(self, instructions):
        ...
        while (i < length):
            if instructions[i].opname == 'FOR_ITER':
                ...
                j = i - 1
                while (instructions[j].opname != 'LOAD_ATTR' and instructions[j].opname != 'LOAD_CONST' and instructions[j].opname != 'LOAD_FAST'):
                    j -= 1
                if(instructions[j].opname == 'LOAD_ATTR' or instructions[j].opname == 'LOAD_FAST'):
                    if(instructions[j].argval == '__dict__'):
                        value = self.code_sync_obj['dict']
                    else:
                        value = self.code_sync_obj[instructions[j].argval]
